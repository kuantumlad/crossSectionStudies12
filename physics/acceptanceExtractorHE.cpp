...
void  WriteAcceptanceProjectionY( TH2F*, TH2F*, int, const char* );
...
TGraphErrors* AcceptancePerSector(TH2F *h_rec_in, TH2F *h_gen_in){
  std::cout << " Calculating Acceptance Per Sector for histogram " << h_rec_in->GetTitle() << std::endl;
  TH1F *h_rec = (TH1F*)h_rec_in->ProjectionY();
  
  int n_bins_filled = -1;
  for( int bb = 0; bb < h_rec_in->GetNbinsX(); bb++ ){    
    int current_content = h_rec_in->GetBinContent(bb,10); // scan over x bins at y_bin 10
  ...
  for( int b = 1; b < rec_bins; b++ ){
    ...
    if( theta_center < 6.0 || theta_center > 20.0 ) continue; 
    double accp_ratio = h_rec->GetBinContent(b)/h_gen->GetBinContent(b);    
    //double accp_ratio_err = accp_ratio * sqrt( h_rec->GetBinContent(b)/pow(h_gen->GetBinContent(b),2)  + pow(h_rec->GetBinContent(b),2)/pow(h_gen->GetBinContent(b),3));
    double accp_ratio_err =1.0/h_gen->GetBinContent(b)  * sqrt( h_rec->GetBinContent(b)*h_rec->GetBinContent(b) / h_gen->GetBinContent(b) + h_rec->GetBinContent(b));
    if(h_gen->GetBinContent(b) == 0 ) accp_ratio = 0.0;
...
TGraphErrors* AcceptanceProjectionY( TH2F *hr, TH2F *hg ){
  

  std::cout << " Getting Acceptance for " << hr->GetTitle() << " along Y " <<  std::endl; 
  TH1D *hry = hr->ProjectionY(hr->GetTitle());
  TH1D *hgy = hg->ProjectionY(hg->GetTitle());

  int nbin = hry->GetNbinsX();
  std::cout << " number of bins along projectionY " << nbin << std::endl;
  ...
  for( int ii =0; ii <nbin; ii++ ){
    double hr_count = (double)hry->GetBinContent(ii);
    // divide gen by 6 to get approximate generted events per sector
    double hg_count = (double)hgy->GetBinContent(ii)/6.0;
    double accp = hr_count/hg_count;
    double accp_ratio_err =1.0/hgy->GetBinContent(ii)  * sqrt( hry->GetBinContent(ii)*hry->GetBinContent(ii) / hgy->GetBinContent(ii) + hry->GetBinContent(ii)); 
    std::cout << " bin " << ii << " rec bin center " << hry->GetBinCenter(ii) << " hr_count " << hr_count << " gen bin center " << hgy->GetBinCenter(ii) <<"  hg_count "<< hg_count << " accp " << accp  << " err " << accp_ratio_err << std::endl;      
    if( hr_count == 0 || hg_count == 0 ){ accp=0.0; accp_ratio_err=0.0; }

    v_b_center.push_back(hry->GetBinCenter(ii) );
...
void WriteAcceptanceProjectionY( TH2F *hr, TH2F *hg, int run, const char* field ){
  ...
  std::string f_out_name = parentDirectory+"elastic_projY_theta_acceptance_"+std::to_string(run)+"_f"+field+".txt";
  std::string f_out_name_err = parentDirectory+"elastic_projY_theta_acceptance_error_"+std::to_string(run)+"_f"+field+".txt";
  ...
  for( int ii =0; ii <nbin; ii++ ){
    ...
    outputAcceptance << ii << " " << accp << std::endl; // " \n";
    outputAcceptanceError << ii << " " << accp_ratio_err << std::endl;//" ";
  }
  outputAcceptance.close();
  outputAcceptanceError.close();  
...
int acceptanceExtractorHE(const char* inFileData, int run, const char* field_config ){ // const char* outfile, int run ){
  ...
  for ( int s = 0; s < 6; s++ ){
    ...
    TH2F *h_gen_temp = (TH2F*)fData->Get("mc/h_mc_el_wtheta");
    std::cout << " getting acceptance for sector " << s << std::endl;
    TGraphErrors *g_accp_sector = AcceptanceProjectionY(h_rec_temp, h_gen_temp);   
    WriteAcceptanceProjectionY( h_rec_temp, h_gen_temp, run, field_config );
