...
TGraphErrors* AcceptanceProjectionY( TH2F *hr, TH2F *hg, bool per_sector ){
  ...
  for( int ii =0; ii <nbin; ii++ ){
    ...
    double hg_count = (double)hgy->GetBinContent(ii)/sector_scale;
...
void WriteAcceptanceProjectionY( TH2F *hr, TH2F *hg, int run, const char* field, int sector ){
  ...
  std::string f_out_name = parentDirectory+"elastic_projY_theta_acceptance_s"+std::to_string(sector)+"_"+std::to_string(run)+"_f"+field+".txt";
  std::string f_out_name_err = parentDirectory+"elastic_projY_theta_acceptance_error_s"+std::to_string(sector)+"_"+std::to_string(run)+"_f"+field+".txt";
...
int acceptanceExtractorDISv0(const char* inFileData, int run, const char* field_config ){ // const char* outfile, int run ){
  ...
  // Get the acceptance for difference bin sizes in theta 
  TCanvas* c_g_accp_bb = new TCanvas("c_g_accp_bb","c_g_accp_bb",900,900);
