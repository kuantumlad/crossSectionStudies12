...
void disPlotterComparison(const char* infile, const char* mcfile, const char* config, int run){
  ...
  //// plot w spectrum - compare GENERATOR to DATA 
  TCanvas *c2 = new TCanvas("c2","c2",1800,1200);
  c2->Divide(3,2);
  TH2F* h2_el_wq2_gen = (TH2F*)fin->Get(Form("mc/hist_mc_all_electron_q2_vs_w"));
  //TH2F* h2_el_wq2_gen_cut = (TH2F*)fin->Get(Form("mc/hist_mc_all_electron_q2_vs_w_ycut"));
  TH1D* h_el_w_gen = h2_el_wq2_gen->ProjectionX();
  //TH1F* h_el_w_gen_ycut = h2_el_wq2_gen_ycut->ProjectionX();
  ...
  for( int s = 1; s <= 6; s++ ){
    ...
    h_el_w->Draw("HIST");
    std::cout << " scaling histogram by " << h_el_w->GetMaximum()/h_el_w_gen->GetMaximum()  << std::endl;
    //reverse when there is more simulation
    h_el_w_gen->Scale( h_el_w_gen->GetMaximum()/h_el_w->GetMaximum() );
    h_el_w_gen->SetLineColor(kBlue);
    h_el_w->SetLineColor(kRed);
    h_el_w_gen->Draw("SAME HIST");
  ...
  c2->SaveAs(Form("h_el_compare_gen_data_w_dis_%s.pdf",config));

  //// plot w spectrum 
  TCanvas *c2a = new TCanvas("c2a","c2a",1800,1200);
  c2a->Divide(3,2);
  ...
  for( int s = 1; s <= 6; s++ ){
    ...
    c2a->cd(s);
    TH1F* h_el_w = (TH1F*)fin->Get(Form("kinematics/h_el_w_s%d",s));
    TH1F* h_el_w_sim = (TH1F*)fmc->Get(Form("kinematics/h_el_w_s%d",s));
    h_el_w->SetTitle(Form("W Sector %d",s));
    h_el_w->GetXaxis()->SetTitle("W (GeV)");
    h_el_w->SetLineColor(kRed);
    h_el_w_sim->SetLineColor(kBlue);
    h_el_w->Draw("HIST");
    h_el_w_sim->Draw("SAME HIST");
    std::cout << " scaling histogram by " << h_el_w->GetMaximum()/h_el_w_sim->GetMaximum()  << std::endl;
    //reverse when there is more simulation
    h_el_w_sim->Scale( h_el_w->GetMaximum()/h_el_w_sim->GetMaximum() );
    

    c2a->Update();
    TVirtualPad *p1 = c2a->GetPad(1);
    double ymax = gPad->GetUymax();
    double ymin = gPad->GetUymin();
    std::cout << ymax << std::endl;
    TLine *l_deltaP = new TLine(1.232,ymin, 1.232, ymax);
  ...
  c2a->SaveAs(Form("h_el_compare_sim_data_w_dis_%s.pdf",config));
  ...
  TH1F *h_w_temp = (TH1F*)fmc->Get(Form("kinematics/h_el_w_s%d",1));
  int w_temp_bins = h_w_temp->GetXaxis()->GetNbins();
  std::vector<TH1F*> h_w_per_sect;
  TH1F *h_el_w_all_sect = new TH1F("h_el_w_mcall_sect","h_el_w_mcall_sect",h_w_temp->GetXaxis()->GetNbins(),h_w_temp->GetXaxis()->GetBinLowEdge(1), h_w_temp->GetXaxis()->GetBinUpEdge(w_temp_bins));   
  for( int s = 1; s <= 6; s++ ){    
    //c1->SetGrid();
    TH1F* h_el_w_sect = (TH1F*)fmc->Get(Form("kinematics/h_el_w_s%d",s));
  ...
  for( int ss = 0; ss < h_w_per_sect.size(); ss++ ){
    ...
    TH1F *h_temp = (TH1F*)h_w_per_sect[ss]->Clone(Form("htemp_el_mcw_s%d",ss));

    h_temp->SetTitle(Form("SIM Ratio of Events in Sector %d to All",ss));
  ...
  c3->SaveAs("h_el_compare_sim_w_ratio_to_all_sect.pdf");
