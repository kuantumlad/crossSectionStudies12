...
///
///  Then call function:  filter_clas12("data_pass0/run_4013_1.root", "output/4013_1.root", 4013)
///                       filter_clas12("data_pass0/2391_1.root", "output/2391_1.root", 2391)
///                       filter_clas12("data_pass0/2587_1.root", "output/2587_1.root", 2587)
///
///
///                       filter_clas12_may("data_sim/sidis/sim_sidis_inbending.root", "output/sim_sidis_inbending_out.root", 11)
///                       filter_clas12_may("data_sim/sidis/sim_sidis_outbending.root", "output/sim_sidis_outbending_out.root", 11)
//
...
bool use_FT = false;
bool use_FD = true;
bool use_CD = false;//true;

bool outbending = true;
...
bool population_weighting = true;
...
bool population_weighting_CD = true;
...
bool use_own_beta_charged = true;
bool use_own_beta_neutrals = false;
bool use_own_beta_charged_FT = true;
...
/// List of CLAS12 hipo branches
    ...
    //raw events
   vector<short>             *vCrate   = 0; 
   vector<short>             *vSlot    = 0;
   ...
   vector<short>  *vpart_status  = 0;     // indicated FT, FD or CD
   ...
   vector<int>    *vMC_pid = 0;     
   vector<float>  *vMC_px  = 0;    
   vector<float>  *vMC_py  = 0;     
   vector<float>  *vMC_pz  = 0;     
   vector<float>  *vMC_vx  = 0;    
   vector<float>  *vMC_vy  = 0;
   vector<float>  *vMC_vz  = 0;
...
//////////////////////////////////////////////////////////////////////////////////////
////////////////
// set the cut parameters for the sampling fraction variables here
...
/// general particle properties and additional particle bank properties for non identified particles:
  ...
  int   partMC_charge[BUFFER], partMC_pid[BUFFER];
...
// --> vectors are used to optimize the file size
  ...
  vector<double> p4_prot_vx;
  vector<double> p4_prot_vy;
  vector<double> p4_prot_vz;
...
void set_cut_parameters(std::string inPath, int run);
void get_faraday_cup(void);

// raw particle assignment

void assign_particles(void);
void FC_accumulated_charge(void);
...
//Int_t filter_clas12( Char_t *inFile, Char_t *outputfile, int run)
  Char_t *inFile = argv[1];
  Char_t *outputfile = argv[2];
...
/// /////////////////////////////////////////////////////////////////////////////
    ...
    TTree mc_tree("mc_tree", "mc_tree");
    mc_tree.Branch("p4_mc_pid", &vMC_pid);
    mc_tree.Branch("p4_mc_px", &vMC_px);
    mc_tree.Branch("p4_mc_py", &vMC_py);
    mc_tree.Branch("p4_mc_pz", &vMC_pz);
    mc_tree.Branch("p4_mc_vx", &vMC_vx);
    mc_tree.Branch("p4_mc_vy", &vMC_vy);
    mc_tree.Branch("p4_mc_vz", &vMC_vz);

    TTree scaler_tree("scaler_tree", "scaler_tree");
    scaler_tree.Branch("RAW_scaler_crate", &vCrate);
    scaler_tree.Branch("RAW_scaler_slot", &vSlot);
    scaler_tree.Branch("RAW_scaler_channel", &vChannel);
    scaler_tree.Branch("RAW_scaler_value", &vValue);
    ...
    out_tree.Branch("p4_prot_vx", &p4_prot_vx);
    out_tree.Branch("p4_prot_vy", &p4_prot_vy);
    out_tree.Branch("p4_prot_vz", &p4_prot_vz);
...
/// ////////////////////////////////////////////////////////////////////////////
   ...
   anaTree->SetBranchAddress("RAW_scaler_crate", &vCrate);
...
if(MC_Particle){
   anaTree->SetBranchAddress("MC_Particle_pid", &vMC_pid);  
   anaTree->SetBranchAddress("MC_Particle_px", &vMC_px);
   anaTree->SetBranchAddress("MC_Particle_py", &vMC_py);
   anaTree->SetBranchAddress("MC_Particle_pz", &vMC_pz);
   anaTree->SetBranchAddress("MC_Particle_vx", &vMC_vx);
   anaTree->SetBranchAddress("MC_Particle_vy", &vMC_vy);
   anaTree->SetBranchAddress("MC_Particle_vz", &vMC_vz);
}
...
out->cd ("missing_mass");
  ...
  hist_e_p_mismass2 = new TH1F("hist_e_p_mismass2", "e p --> e p X missing mass squared", 500, -0.5, 1);   
  hist_e_p_mismass2->GetXaxis()->SetTitle("missing mass /GeV");
  hist_e_p_mismass2->GetYaxis()->SetTitle("counts");
  hist_e_pi0_mismass = new TH1F("hist_e_pi0_mismass", "e p --> e #pi^{0} X missing mass", 500, 0.0, 5.0);   
  hist_e_pi0_mismass->GetXaxis()->SetTitle("missing mass /GeV");
  hist_e_pi0_mismass->GetYaxis()->SetTitle("counts");
  hist_e_pip_mismass = new TH1F("hist_e_pip_mismass", "e p --> e #pi^{+} X missing mass", 500, 0.0, 5.0);   
  hist_e_pip_mismass->GetXaxis()->SetTitle("missing mass /GeV");
  hist_e_pip_mismass->GetYaxis()->SetTitle("counts");
  hist_e_pim_mismass = new TH1F("hist_e_pim_mismass", "e p --> e #pi^{-} X missing mass", 500, 0.0, 5.0);   
  ...
  for(Int_t i = 0; i < 6; i++){
    ...
    hist_e_p_mismass2_sec[i] = new TH1F(name, title, 500, -0.5, 1);   
    ...
    hist_e_pi0_mismass_sec[i] = new TH1F(name, title, 500, 0.0, 5.0);   
    ...
    hist_e_pip_mismass_sec[i] = new TH1F(name, title, 500, 0.0, 5.0);   
    ...
    sprintf(title,"e p --> e #pi^{-} X missing mass sector %01d", i+1);
    hist_e_pim_mismass_sec[i] = new TH1F(name, title, 500, 0.0, 5.0);   
...
out->cd ("sampling_fraction");
  ...
  hist_sampling_fraction_vs_E_sec1 = new TH2F("sampling_fraction_vs_E_sec1", "EC total sampling fraction versus E sec 1", 350,0,Ebeam+shift, 300,0,0.6);   
  hist_sampling_fraction_vs_E_sec1->GetXaxis()->SetTitle("E /GeV");
  hist_sampling_fraction_vs_E_sec1->GetYaxis()->SetTitle("E/p");
  hist_sampling_fraction_vs_E_sec2 = new TH2F("sampling_fraction_vs_E_sec2", "EC total sampling fraction versus E sec 2", 350,0,Ebeam+shift, 300,0,0.6);   
  hist_sampling_fraction_vs_E_sec2->GetXaxis()->SetTitle("E /GeV");
  hist_sampling_fraction_vs_E_sec2->GetYaxis()->SetTitle("E/p");
  hist_sampling_fraction_vs_E_sec3 = new TH2F("sampling_fraction_vs_E_sec3", "EC total sampling fraction versus E sec 3", 350,0,Ebeam+shift, 300,0,0.6);   
  hist_sampling_fraction_vs_E_sec3->GetXaxis()->SetTitle("E /GeV");
  hist_sampling_fraction_vs_E_sec3->GetYaxis()->SetTitle("E/p");
  hist_sampling_fraction_vs_E_sec4 = new TH2F("sampling_fraction_vs_E_sec4", "EC total sampling fraction versus E sec 4", 350,0,Ebeam+shift, 300,0,0.6);   
  hist_sampling_fraction_vs_E_sec4->GetXaxis()->SetTitle("E /GeV"); 
  hist_sampling_fraction_vs_E_sec4->GetYaxis()->SetTitle("E/p");
  hist_sampling_fraction_vs_E_sec5 = new TH2F("sampling_fraction_vs_E_sec5", "EC total sampling fraction versus E sec 5", 350,0,Ebeam+shift, 300,0,0.6);   
  hist_sampling_fraction_vs_E_sec5->GetXaxis()->SetTitle("E /GeV");
  hist_sampling_fraction_vs_E_sec5->GetYaxis()->SetTitle("E/p");
  hist_sampling_fraction_vs_E_sec6 = new TH2F("sampling_fraction_vs_E_sec6", "EC total sampling fraction versus E sec 6", 350,0,Ebeam+shift, 300,0,0.6);   
...
cout << endl;
 ...
 std::cout << " setting cut parameters " << std::endl;
...
for(Int_t i = 0; i < BUFFER; i++){ 
  ...
  partMC_vx[i] = 0; partMC_vy[i] = 0; partMC_vz[i] = 0; partMC_charge[i] = 0; partMC_pid[i] = 0;
...
if(Helic == 0) helicity = -1;
if(Helic == 1) helicity = +1;
 fcup = BCG;
assign_particles();

 FC_accumulated_charge();
...
if(e_count > 0){
  ...
  for(Int_t i = 0; i < BUFFER; i++){
    if(partMC_pid[i] == 11)  { e_MCcount   += 1;}  
    if(partMC_pid[i] == 2212){ p_MCcount   += 1;}  
    if(partMC_pid[i] == 2112){ n_MCcount   += 1;}  
    if(partMC_pid[i] == 211) { pip_MCcount += 1;}  
    if(partMC_pid[i] == -211){ pim_MCcount += 1;}  
    if(partMC_pid[i] == 321) { Kp_MCcount  += 1;}  
    if(partMC_pid[i] == -321){ Km_MCcount  += 1;} 
  }
  ...
  if(f_e_p_miss.M() != 0 && p_count > 0) hist_e_p_mismass->Fill(f_e_p_miss.M());
  if(f_e_p_miss.M() != 0 && p_count > 0) hist_e_p_mismass2->Fill(f_e_p_miss.M2());
  if(f_e_pi0_miss.M() != 0) hist_e_pi0_mismass->Fill(f_e_pi0_miss.M());
  if(f_e_pip_miss.M() != 0 && pip_count > 0) hist_e_pip_mismass->Fill(f_e_pip_miss.M());
  if(f_e_pim_miss.M() != 0 && pim_count > 0) hist_e_pim_mismass->Fill(f_e_pim_miss.M());

  for(Int_t i = 0; i < 6; i++){
    if(e_FTOF_sec[0] == i+1 && f_e_p_miss.M() != 0 && p_count > 0) hist_e_p_mismass_sec[i]->Fill(f_e_p_miss.M());
    if(e_FTOF_sec[0] == i+1 && f_e_p_miss.M() != 0 && p_count > 0) hist_e_p_mismass2_sec[i]->Fill(f_e_p_miss.M2());
    if(e_FTOF_sec[0] == i+1 && f_e_pi0_miss.M() != 0) hist_e_pi0_mismass_sec[i]->Fill(f_e_pi0_miss.M());
    if(e_FTOF_sec[0] == i+1 && f_e_pip_miss.M() != 0 && pip_count > 0) hist_e_pip_mismass_sec[i]->Fill(f_e_pip_miss.M());
    if(e_FTOF_sec[0] == i+1 && f_e_pim_miss.M() != 0 && pim_count > 0) hist_e_pim_mismass_sec[i]->Fill(f_e_pim_miss.M());
  }
...
// default PID  (neg. charge is fulfilled for all)
  ...
  if(FD_eid_default_PID_check[i]){  
    ...
    //std::cout << " > >  " << part_Cal_PCAL_sector[i] << " " << part_p[i] << std::endl;
...
/// define calorimeter fiducial cut borders
  ...
  if(FD_eid_default_PID_check[i]){ 
    ...
    if(part_p[i] > 3 &&  part_p[i] < 6){
      ...
      if(part_Cal_PCAL_sector[i] == 1 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_sec1->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 2 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_sec2->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 3 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_sec3->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 4 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_sec4->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 5 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_sec5->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 6 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_sec6->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);

      if(part_Cal_PCAL_sector[i] == 1 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_sec1->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 2 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_sec2->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 3 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_sec3->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 4 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_sec4->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 5 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_sec5->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
      if(part_Cal_PCAL_sector[i] == 6 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_sec6->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i]);
    }
    ...
    if(part_Cal_PCAL_sector[i] == 1 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_vs_p_sec1->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 2 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_vs_p_sec2->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 3 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_vs_p_sec3->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 4 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_vs_p_sec4->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 5 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_vs_p_sec5->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 6 && part_p[i] > 0) hist_electron_sampfrac_vs_v_coord_vs_p_sec6->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);

    if(part_Cal_PCAL_sector[i] == 1 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_vs_p_sec1->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 2 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_vs_p_sec2->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 3 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_vs_p_sec3->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 4 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_vs_p_sec4->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 5 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_vs_p_sec5->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
    if(part_Cal_PCAL_sector[i] == 6 && part_p[i] > 0) hist_electron_sampfrac_vs_w_coord_vs_p_sec6->Fill(part_Cal_PCAL_lu[i], part_Cal_energy_total[i]/part_p[i], part_p[i]);
  }
...
for(Int_t i = 0; i < e_count; i++){ 
  ...
  if(p4_ele_raw[i].Theta()*180/Pival > 5 && kinW > 0) hist_W_raw->Fill(kinW); 
...
/// /////////////////////////////////////////////////////////////
 ...
 std::cout << " >> %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% " << std::endl;
 std::cout << " >> Total accumulated beam charge in FC: " << tot_beam_charge << std::endl;
 std::cout << " >> %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% " << std::endl;
...
TH2F* create_hist_EC_total_sampling_fraction_sec1(const int cutnum){
  ...
  hist_EC_total_sampling_fraction_sec1[cutnum] = new TH2F(name, name, 1100,0.0, Ebeam + energy_shift, 600,0,0.6);    // sampling fraction plots
...
TH2F* create_hist_EC_total_sampling_fraction_sec2(const int cutnum){
  ...
  hist_EC_total_sampling_fraction_sec2[cutnum] = new TH2F(name, name, 1100,0, Ebeam + energy_shift, 600,0,0.6);   
...
TH2F* create_hist_EC_total_sampling_fraction_sec3(const int cutnum){
  ...
  hist_EC_total_sampling_fraction_sec3[cutnum] = new TH2F(name, name, 1100,0, Ebeam + energy_shift, 600,0,0.6);   
...
TH2F* create_hist_EC_total_sampling_fraction_sec4(const int cutnum){
  ...
  hist_EC_total_sampling_fraction_sec4[cutnum] = new TH2F(name, name, 1100,0, Ebeam + energy_shift, 600,0,0.6);   
...
TH2F* create_hist_EC_total_sampling_fraction_sec5(const int cutnum){
  ...
  hist_EC_total_sampling_fraction_sec5[cutnum] = new TH2F(name, name, 1100,0, Ebeam + energy_shift, 600,0,0.6);   
...
TH2F* create_hist_EC_total_sampling_fraction_sec6(const int cutnum){
  ...
  hist_EC_total_sampling_fraction_sec6[cutnum] = new TH2F(name, name, 1100,0, Ebeam + energy_shift, 600,0,0.6);   
...
void set_cut_parameters(std::string parm_path, int run){
  ...
  for( int i = 0; i < p0_mean.size(); i++ ){
    std::cout<< " > ec sf mean sector " << i << " " << p0_mean[i] <<  " " << p1_mean[i] << " " << p2_mean[i] << " " << p3_mean[i] << std::endl;
  }
  for( int i = 0; i < p0_sigma.size(); i++ ){
    std::cout<< " > ec sf sigma cuts  sector " << i << " " << p0_sigma[i] <<  " " << p1_sigma[i] << std::endl;
  }
...
/// ///////////////////////////////////////////////////////////////////////////////////////////////////////
/// assign faraday cup information
...
void assign_particles(void){
  ...
  Scaler_Nentries = vChannel->size();
  if(MC_Particle) MC_Nentries   = vMC_pid->size();

  //raw scaler bank 
  /*
  if (Scaler_Nentries > 0){
    for(int i = 0; i < Cal_Nentries; i++){
      channel.push_back(vChannel->at(i));
      crate.push_back(vCrate->at(i));
      slot.push_back(vSlot->at(i));
      value.push_back(vValue->at(i));
      }
  }
  */
  ...
  if(Traj_Nentries > 0){
    for(int i = 0; i < Traj_Nentries; i++){
      if(vTraj_pindex->at(i) >= 0 && vTraj_pindex->at(i) < BUFFER && vTraj_detID->at(i) == 12){    
      ...
      if(vTraj_pindex->at(i) >= 0 && vTraj_pindex->at(i) < BUFFER && vTraj_detID->at(i) == 24){  
      ...
      if(vTraj_pindex->at(i) >= 0 && vTraj_pindex->at(i) < BUFFER && vTraj_detID->at(i) == 36){  
  ...
  if(MC_Nentries > 0){
    for(int i = 0; i < MC_Nentries; i++){
      if(i < BUFFER){ 
        partMC_pid[i] = vMC_pid->at(i);
        partMC_px[i] = vMC_px->at(i);
        partMC_py[i] = vMC_py->at(i);
        partMC_pz[i] = vMC_pz->at(i);
        partMC_vx[i] = vMC_vx->at(i);
        partMC_vy[i] = vMC_vy->at(i);
        partMC_vz[i] = vMC_vz->at(i);
...
void select_electron(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    if( i < BUFFER){
      ...
      if(FD_eid_default_PID_check[i] == true &&  part_theta[i]*180/Pival > 5.0) 	    						FD_eid_default_PID_pass += 1;
      if(FD_eid_charge_check[i]  == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0)				FD_eid_charge_pass += 1;
      ...
      if(FD_eid_default_PID_check[i] && FD_eid_EC_sampling_fraction_check[i] && FD_eid_charge_check[i] 
      && FD_eid_default_PID_check[i] && part_theta[i]*180/Pival>5.0)									FD_eid_EC_sampling_fraction_pass += 1;
      ...
      for(int k = 0; k < Npart; k++){
        if(k < BUFFER){
          ...
          if(use_own_PID_electron == true){ 
            selector = (FD_eid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37)|| (FT_eid_all_check[k] && part_theta[k]*180/Pival > 2.5 && part_theta[k]*180/Pival < 4.5); 
          }
          else{ 
            selector = (FD_eid_default_PID_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (FT_eid_PID_check[k]  && part_theta[k]*180/Pival > 2.5 
                        && part_theta[k]*180/Pival < 4.5); 
          }       
...
void select_proton(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_protid_default_PID_check[i]  == true &&  part_theta[i]*180/Pival > 5.0 &&  part_theta[i]*180/Pival < 35.0) 		FD_protid_default_PID_pass += 1;
    if(FD_protid_charge_check[i]   == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_protid_charge_pass += 1;
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_proton == true){ 
          selector = (FD_protid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_protid_all_check[k] && part_theta[k]*180/Pival > 35); 
        }
        else{ 
	  selector = (CD_protid_charge_check[k] && part_theta[k]*180/Pival > 35 && vpart_status->at(k) > 3500);  
	  //selector = (CD_protid_beta_check[k] && part_theta[k]*180/Pival > 35 && vpart_status->at(k) > 3500);
  ...
  for(int i = 0; i < BUFFER; i++){
    if(p_ind[i] != -1){
      ...
      if (vpart_status->at(p_ind[i]) > 3500){
	//	cout << "zalupa : " << vpart_status->at(p_ind[i]) <<endl; 
      }
...
void select_neutron(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_neutrid_default_PID_check[i] == true &&  part_theta[i]*180/Pival > 5.0 &&  part_theta[i]*180/Pival < 35.0) 	FD_neutrid_default_PID_pass += 1;
    if(FD_neutrid_charge_check[i]  == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 	FD_neutrid_charge_pass += 1;
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_neutron == true){ 
          selector = (FD_neutrid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_neutrid_all_check[k] && part_theta[k]*180/Pival > 35); 
        }
        else{ 
          selector = (FD_neutrid_default_PID_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_neutrid_default_PID_check[k] && part_theta[k]*180/Pival > 35); 
        }
...
void select_pip(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_pipid_default_PID_check[i]  == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_pipid_default_PID_pass += 1;
    if(FD_pipid_charge_check[i]  == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_pipid_charge_pass += 1;
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_pip == true){ 
          selector = (FD_pipid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_pipid_all_check[k] && part_theta[k]*180/Pival > 35); 
        }
        else{ 
          selector = (FD_pipid_default_PID_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_pipid_default_PID_check[k] && part_theta[k]*180/Pival > 35); 
        }
...
void select_pim(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_pimid_default_PID_check[i]  == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_pimid_default_PID_pass += 1;
    if(FD_pimid_charge_check[i]  == true  &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_pimid_charge_pass += 1;
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_pim == true){ 
          selector = (FD_pimid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_pimid_all_check[k] && part_theta[k]*180/Pival > 35); 
        }
        else{ 
          selector = (FD_pimid_default_PID_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_pimid_default_PID_check[k] && part_theta[k]*180/Pival > 35); 
        }
...
void select_Kplus(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_Kpid_default_PID_check[i]  == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 	FD_Kpid_default_PID_pass += 1;
    if(FD_Kpid_charge_check[i]   == true &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 	FD_Kpid_charge_pass += 1;
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_Kp == true){ 
          selector = (FD_Kpid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_Kpid_all_check[k] && part_theta[k]*180/Pival > 35); 
        }
        else{ 
          selector = (FD_Kpid_default_PID_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_Kpid_default_PID_check[k] && part_theta[k]*180/Pival > 35); 
        }
...
void select_Kminus(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_Kmid_default_PID_check[i]  == true &&  part_theta[i]*180/Pival > 5.0 &&  part_theta[i]*180/Pival < 35.0) 	FD_Kmid_default_PID_pass += 1;
    if(FD_Kmid_charge_check[i]  == true   &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 	FD_Kmid_charge_pass += 1;
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_Km == true){ 
          selector = (FD_Kmid_all_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_Kmid_all_check[k] && part_theta[k]*180/Pival > 35); 
        }
        else{ 
          selector = (FD_Kmid_default_PID_check[k] && part_theta[k]*180/Pival > 4.5 && part_theta[k]*180/Pival < 37) || (CD_Kmid_default_PID_check[k] && part_theta[k]*180/Pival > 35); 
        }
...
void select_photon(int run){
  ...
  for(Int_t i = 0; i < Npart; i++){
    ...
    if(FD_photid_default_PID_check[i] &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_photid_default_PID_pass += 1;
    if(FD_photid_charge_check[i] &&  part_theta[i]*180/Pival > 5.0  &&  part_theta[i]*180/Pival < 35.0) 		FD_photid_charge_pass += 1;
    ...
    if(use_FT == false){ FT_photid_all_check[i] = false; theta_min = 5;}
    ...
    if( i < BUFFER){
      ...
      for(int k = 0; k < Npart; k++){
        ...
        if(use_own_PID_photon == true){ 
          selector = (FD_photid_all_check[k] && part_theta[k]*180/Pival > 5 && part_theta[k]*180/Pival < 35)|| (FT_photid_all_check[k] && part_theta[k]*180/Pival > 2.5 && part_theta[k]*180/Pival < 4.5 
                                                                                                                && part_p[k] > 0.1); 
        }
        else{ 
          selector = (FD_photid_default_PID_check[k] && part_theta[k]*180/Pival > 5 && part_theta[k]*180/Pival < 35) || (FT_photid_PID_check[k]  && part_theta[k]*180/Pival > 2.5 
                      && part_theta[k]*180/Pival < 4.5); 
        }       
...
bool EC_sampling_fraction_cut(int j){
  ...
  for(Int_t k = 0; k < 6; k++){  
    if(part_Cal_PCAL_sector[j]-1 == k){
      mean = p0_mean[k] *( 1 + part_p[j]/sqrt(pow(part_p[j],2) + p1_mean[k])) + p2_mean[k] * part_p[j] + p3_mean[k] * pow(part_p[j],2);
      sigma = p0_sigma[k] + p1_sigma[k] / sqrt(part_p[j]);
...
bool DC_hit_position_region1_fiducial_cut(int j){
  ...
  //std::cout << " x1 " << x1_rot << " y1 " << y1_rot << std::endl;
  //std::cout << " left cut "  << left << " right cut " << right << std::endl;
...
bool DC_hit_position_region1_fiducial_cut_hadrons_positive(int j){
  ...
  double radius2_DCr1 = pow(25,2)-pow(y1_rot,2);    // cut out the inner circle
...
bool DC_hit_position_region3_fiducial_cut_hadrons_positive(int j){
  ...
  double radius2_DCr3 = pow(77,2)-pow(y3_rot,2);    // cut out the inner circle
...
bool DC_hit_position_region1_fiducial_cut_hadrons_negative(int j){
  ...
  double radius2_DCr1 = pow(32,2)-pow(y1_rot,2);    // cut out the inner circle
...
bool DC_hit_position_region3_fiducial_cut_hadrons_negative(int j){
  ...
  double radius2_DCr3 = pow(52,2)-pow(y3_rot,2);    // cut out the inner circle
...
bool prot_beta_cut(int j, int run){
  ...
  for(Int_t k = 0; k < 6; k++){  
    if(part_FTOF_sector_layer2[j]-1 == k){
      ...
      sigma = prot_sigma_p0[k] + prot_sigma_p1[k]/sqrt(part_p[j]);
...
bool pip_beta_cut(int j, int run){
  ...
  for(Int_t k = 0; k < 6; k++){  
    if(part_FTOF_sector_layer2[j]-1 == k){
      ...
      sigma = pip_sigma_p0[k] + pip_sigma_p1[k]/sqrt(part_p[j]);
      upper_lim = mean + sigma_range * sigma;
      lower_lim = mean - sigma_range * sigma;
    }
  }
...
bool pim_beta_cut(int j, int run){
  ...
  for(Int_t k = 0; k < 6; k++){  
    if(part_FTOF_sector_layer2[j]-1 == k){
      ...
      sigma = pim_sigma_p0[k] + pim_sigma_p1[k]/sqrt(part_p[j]);
      upper_lim = mean + sigma_range * sigma;
      lower_lim = mean - sigma_range * sigma;
    }
  }
...
bool Kp_beta_cut(int j, int run){
  ...
  for(Int_t k = 0; k < 6; k++){  
    if(part_FTOF_sector_layer2[j]-1 == k){
      ...
      sigma = Kp_sigma_p0[k] + Kp_sigma_p1[k]/sqrt(part_p[j]);
      upper_lim = mean + sigma_range * sigma;
      lower_lim = mean - sigma_range * sigma;
    }
  }
...
bool Km_beta_cut(int j, int run){
  ...
  for(Int_t k = 0; k < 6; k++){  
    if(part_FTOF_sector_layer2[j]-1 == k){
      ...
      sigma = Km_sigma_p0[k] + Km_sigma_p1[k]/sqrt(part_p[j]);
      upper_lim = mean + sigma_range * sigma;
      lower_lim = mean - sigma_range * sigma;
    }
  }
...
bool prot_delta_beta_cut(int j, int run){
  ...
  double mean = 0.001605;
  double sigma = 0.01417;
  double delta_beta_min = mean - 3 * sigma;
  double delta_beta_max = mean + 3 * sigma;
...
bool pip_delta_beta_cut(int j, int run){
  ...
  double mean = -0.003453;
  double sigma = 0.009959;
  double delta_beta_min = mean - 3 * sigma;
  double delta_beta_max = mean + 3 * sigma;
...
bool pim_delta_beta_cut(int j, int run){
  ...
  double mean = -0.004294;
  double sigma = 0.009826;
  double delta_beta_min = mean - 3 * sigma;
  double delta_beta_max = mean + 3 * sigma;
...
bool Km_delta_beta_cut(int j, int run){
  ...
  double mean = -0.004764;
  double sigma = 0.007736;
  double delta_beta_min = mean - 3 * sigma;
  double delta_beta_max = mean + 3 * sigma;
...
bool prot_tofmass_cut(int j, int run){
  ...
  double mean = 0.889;
  double sigma = 0.08057;
  double tofmass2_min = mean - 3 * sigma;
  if(tofmass2_min < 0) tofmass2_min = 0;
  double tofmass2_max = mean + 3 * sigma;
  ...
  if(charge > 0){
    ...
    for(Int_t k = 0; k < 6; k++){  
      if(sector-1 == k){
        ...
        double sigma_prot = prot_sigma_p0[k] + prot_sigma_p1[k]/sqrt(part_p[j]);
        ...
        double sigma_pip = pip_sigma_p0[k] + pip_sigma_p1[k]/sqrt(part_p[j]);
        ...
        double sigma_Kp = Kp_sigma_p0[k] + Kp_sigma_p1[k]/sqrt(part_p[j]);
  ...
  if(charge < 0){
    ...
    for(Int_t k = 0; k < 6; k++){
      if(sector-1 == k){
        ...
        double sigma_pim = pim_sigma_p0[k] + pim_sigma_p1[k]/sqrt(part_p[j]);
        ...
        double sigma_Km = Km_sigma_p0[k] + Km_sigma_p1[k]/sqrt(part_p[j]);
...
bool phot_beta_cut(int j, int run){
  if(Beta_neutral(j, run) > 0.9) return true;
  else return false;
}
...
bool phot_EC_hit_position_fiducial_cut(int j){
  ...
  double radius2_PCAL = pow(70,2)-pow(y_PCAL_rot,2);   // cut another 6 cm circle in the inner triangle tip to reject particles influenced by dead metarial

  if(x_PCAL_rot > left_PCAL && x_PCAL_rot > right_PCAL && pow(x_PCAL_rot,2) > radius2_PCAL && x_PCAL_rot < 372) return true;
  else return false;
...
bool CD_prot_beta_cut(int j, int run){
  ...
  if(Beta_charged_central(j, run) <= upper_lim && Beta_charged_central(j, run) >= lower_lim && part_p[j] > 0.3) return true;
  else return false;
}
...
bool CD_pip_beta_cut(int j, int run){
  ...
  if(Beta_charged_central(j, run) <= upper_lim && Beta_charged_central(j, run) >= lower_lim && part_p[j] > 0.7) return true;
  else return false;
}
...
bool CD_maximum_probability_cut(int j, int hypothesis, double conflvl, double anticonflvl, int run){
  ...
  double Kp_mean_p0 = 0.0;
  double Kp_mean_p1 = 1.00000;
  double Kp_mean_p2 = 0.24372;       // lit. Kaon mass
  double Kp_sigma_p0 = -0.00201627;  // copied from pip
  double Kp_sigma_p1 = 0.0171322;    // copied from pip

  double Km_mean_p0 = 0.0;
  double Km_mean_p1 = 1.00000;
  double Km_mean_p2 = 0.24372;       // lit. Kaon mass
  double Km_sigma_p0 = 0.00100331;   // copied from pim
  double Km_sigma_p1 = 0.00882211;   // copied from pim
...
void fill_output_vector_proton(void){
  for(int i = 0; i < BUFFER; i++){
    if(p4_prot[i].E() > 0){
      ...
      p4_prot_vx.push_back(p_vx[i]);
      p4_prot_vy.push_back(p_vy[i]);
      p4_prot_vz.push_back(p_vz[i]);
    }
  } 
}
...
double hadron_sct_corr_central(int j, int run){    // SC timing correction for hadrons
  ...
  double mean_pion_combined_CTOF[] = { 0.108537, 0.127191, 0.140442, 0.112314, 0.0996826, 0.125184, 0.122113, 0.104015, 0.110065, 0.0959113, 0.120144, 0.100694, 0.0959794, 0, 0.102396, 0.107274, 0.110106, 0.0968141, 0.104638, 0.119206, 0.0868256, 0.103452, 0.110534, 0.101279, 0.110514, 0.113945, 0.108079, 0.10452, 0.119515, 0.103479, 0.105117, 0.111425, 0.101853, 0.110747, 0.130012, 0.122349, 0.0974521, 0.0969179, 0.10675, 0.105205, 0.111083, 0.108959, 0.111761, 0.109268, 0.115149, 0.11762, 0.118473, 0.120386};
